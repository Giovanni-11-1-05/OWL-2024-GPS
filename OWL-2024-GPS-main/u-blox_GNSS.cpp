/*
  A Pico Library which allows you to communicate seamlessly with u-blox GNSS modules using the Configuration Interface for the RP Pico
...
{
  // Constructor
  if (debugPin >= 0)  // By default, debugPin is -1, we probably won't be interacting with this.  I have commented this section out as I did not have time to adpat the code
  {
    //gpio_init(debugPin);
...
bool DevUBLOXGNSS::setPacketCfgPayloadSize(size_t payloadSize)
{
  ...
  else if (payloadCfg == nullptr) // Memory has not yet been allocated - so use new
  {
    ...
    {
      if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
        printf("setPacketCfgPayloadSize: packetCfgPayloadSize > spiBufferSize!");
    }
  }
...
uint8_t DevUBLOXGNSS::write_bytes(uint8_t *data, uint8_t length)
{
  return _sfeBus->write_bytes(data, length);
}
// // For SPI, writing bytes will also read bytes simultaneously. Read data is returned in readData
// uint8_t DevUBLOXGNSS::write_read_bytes(const uint8_t *data, uint8_t *readData, uint8_t length)
// {
//   return _sfeBus->write_read_bytes(data, readData, length);
// }
// void DevUBLOXGNSS::start_write_read_byte()
// {
//   _sfeBus->start_write_read_byte();
// }
// void DevUBLOXGNSS::write_read_byte(const uint8_t *data, uint8_t *readData)
// {
//   _sfeBus->write_read_byte(data, readData);
// }
// void DevUBLOXGNSS::write_read_byte(const uint8_t data, uint8_t *readData)
// {
//   _sfeBus->write_read_byte(data, readData);
// }
// void DevUBLOXGNSS::end_write_read_byte()
// {
//   _sfeBus->end_write_read_byte();
// }
// For Serial, attempt Serial.read
// For I2C, read from register 0xFF
// For SPI, read the byte while writing 0xFF
uint8_t DevUBLOXGNSS::read_bytes(uint8_t *data, uint8_t length)
{
  return _sfeBus->read_bytes(data, length);
}
...
bool DevUBLOXGNSS::init(uint16_t maxWait, bool assumeSuccess)
{
  ...
  if (_commType == COMM_TYPE_SPI)
  {
    ...
    if (spiBuffer == nullptr)
    {
      if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      {
        printf("begin (SPI): memory allocation failed for SPI Buffer!");
        return (false);
      }
    }
    else
    {
      ...
      if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      {
        printf("begin (SPI): spiBuffer size is ");
        printf("%d\n", spiBufferSize);
      }
      if ((packetCfgPayloadSize + 8) > spiBufferSize) // Warn the user if spiBuffer is now smaller than the packetCfg payload. Could result in lost data
      {
        printf("begin (SPI): packetCfgPayloadSize > spiBufferSize!");
      }
#endif
    }
  }
...
void DevUBLOXGNSS::setSpiTransactionSize(uint8_t transactionSize)
{
  ...
  else
  {
    ...
    if (_printDebug == true)
    {
      printf("setSpiTransactionSize: you need to call setSpiTransactionSize _before_ begin!");
    }
#endif
  }
}
...
//     if (_printDebug == true)
//     {
//       printf("setSpiBufferSize: you need to call setSpiBufferSize _before_ begin!");
//     }
// #endif
//   }
// }
...
void DevUBLOXGNSS::enableDebugging(bool printLimitedDebug)
{
...
bool DevUBLOXGNSS::checkUbloxI2C(ubxPacket *incomingUBX, uint8_t requestedClass, uint8_t requestedID)
{
  if (to_ms_since_boot(get_absolute_time()) - lastCheck >= i2cPollingWait) 
  {
...
#ifndef SFE_UBLOX_REDUCED_PROG_MEM // Define if not defined
    if (_printDebug == true)
    {
      printf("checkUbloxI2C: %d bytes available\n", bytesAvailable);
    }
#endif
    ...
    while (bytesAvailable)
    {
      ...
      uint8_t bytesReturned = read_bytes(buf, (uint8_t)bytesToRead);
      if ((uint16_t)bytesReturned == bytesToRead)
      {
        for (uint16_t x = 0; x < bytesToRead; x++)
        {
          // -- Not 100% on if this is already valid for Pico
      ...
      else
      {
        ...
        if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
        {
          printf("checkUbloxI2C: bus error? bytesReturned != bytesToRead");
        }
#endif
        return (false);
      }
...
//   start_write_read_byte();

//   uint8_t byteReturned = 0xFF;
//   write_read_byte(0xFF, &byteReturned);
...
void DevUBLOXGNSS::process(uint8_t incoming, ubxPacket *incomingUBX, uint8_t requestedClass, uint8_t requestedID)
{
  ...
  //printf("0x%x\n", incoming); // Echo this byte to the serial port
  ...
  if (currentSentence == SFE_UBLOX_SENTENCE_TYPE_UBX)
  {
      ...
      if (packetBuf.cls != UBX_CLASS_ACK)
      {
        ...
        else if (logBecauseAuto || logBecauseEnabled)
        {
          ...
          if (logBecauseAuto && (maxPayload == 0))
          {
            ...
            if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
            {
                printf("process: autoLookup returned ZERO maxPayload!! Class: 0x%X ID: 0x%X\n", packetBuf.cls, packetBuf.id);
            }
#endif
          }
          ...
          if (payloadAuto == nullptr) // Check if the alloc failed
          {
            ...
            if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
            {
              printf("process: memory allocation failed for \"automatic\" message: Class: 0x%X ID: 0x%X logBecauseAuto:%d logBecauseEnabled:%d\n", packetBuf.cls, packetBuf.id, logBecauseAuto, logBecauseEnabled);
            }
#endif
          ...
          else
          {
            ...
            if (_printDebug == true)
            {
              printf("process: incoming \"automatic\" message: Class: 0x%X ID: 0x%X logBecauseAuto:%d logBecauseEnabled:%d\n", 
              packetBuf.cls, packetBuf.id, logBecauseAuto, logBecauseEnabled);
            }
#endif
          }
    ...
    else if (ubxFrameCounter == 6) // This should be the first byte of the payload unless .len is zero
    {
      if (packetBuf.len == 0) // Check if length is zero (hopefully this is impossible!)
      {
        ...
        if (_printDebug == true)
        {
          printf("checkUbloxI2C: %u bytes available\n", available());
        }
#endif
    ...
    else if (ubxFrameCounter == 7) // This should be the second byte of the payload unless .len is zero or one
    {
          ...
          && (packetBuf.payload[1] == storedID))             // and if the ID matches
      {
        ...
        else // Length is not 2 (hopefully this is impossible!)
        {
          ...
          if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
          {
            printf("process: ACK received with .len != 2: Class: 0x%02X ID: 0x%02X len: %u\n", 
  ...
  else if (currentSentence == SFE_UBLOX_SENTENCE_TYPE_NMEA) // Process incoming NMEA mesages. Selectively log if desired.
  {
    ...
    if (nmeaByteCounter == 5)
    {
...
#endif
      {
        // if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
        // {
        //   printf(F("process: non-auto NMEA message"));
        // }
      }
    ...
    if (nmeaByteCounter == 0) // Check if we are done
    {
      ...
      if (isThisNMEAauto())
      {
        ...
        if (thisChar == '*') // Make sure we found the *
        {
          ...
          {
            printf("hello! NMEA checksum fail (2)!");
          }
        }
        else
        {
      ...
      if (logThisNMEA())
      {
        ...
        if (thisChar == '*') // Make sure we found the *
        {
          ...
          {
            printf("process: _storageNMEA checksum fail!");
          }
        }
      }
    ...
    if (_storageRTCM != nullptr) // Check if RTCM logging storage exists
    {
      ...
      if ((rtcmFrameCounter >= 3) && (rtcmFrameCounter == _storageRTCM->messageLength + 5))
      {
        ...
        if (expectedChecksum == _storageRTCM->rollingChecksum) // Does the checksum match?
        {
          ...
          if (_printDebug == true)
          {
              // Check if the message type is 4072 to decide on appending message subtype
              if (messageType == 4072)
              {
                  printf("process: valid RTCM message type: %d_%d\n", messageType, messageSubType);
              }
              else
              {
                  printf("process: valid RTCM message type: %d\n", messageType);
              }
          }
#endif
        ...
        else
        {
          if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
          {
            printf("process: RTCM checksum fail!");
          }
        }
      }
    }
...
void DevUBLOXGNSS::processUBX(uint8_t incoming, ubxPacket *incomingUBX, uint8_t requestedClass, uint8_t requestedID)
{
  ...
  else if (activePacketBuffer == SFE_UBLOX_PACKET_PACKETAUTO)
  {
    ...
    if (maximum_payload_size == 0)
    {
      ...
      if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      {
          printf("processUBX: autoLookup returned ZERO maxPayload!! Class: 0x%X ID: 0x%X\n", incomingUBX->cls, incomingUBX->id);
      }
#endif
  ...
  else if (incomingUBX->counter == incomingUBX->len + 5) // ChecksumB
  {
    ...
    if ((incomingUBX->checksumA == rollingChecksumA) && (incomingUBX->checksumB == rollingChecksumB))
    {
      ...
      else if ((incomingUBX->cls == UBX_CLASS_ACK) && (incomingUBX->id == UBX_ACK_NACK) && (incomingUBX->payload[0] == requestedClass) && (incomingUBX->payload[1] == requestedID))
      {
        ...
        if (_printDebug == true)
        {
            printf("processUBX: NACK received: Requested Class: 0x%X Requested ID: 0x%X\n", incomingUBX->payload[0], incomingUBX->payload[1]);
        }
#endif
      ...
      else if ((autoLookup(incomingUBX->cls, incomingUBX->id)) || (logThisUBX(incomingUBX->cls, incomingUBX->id)))
      {
        ...
        if (_printDebug == true)
        {
            printf("processUBX: incoming \"automatic\" message: Class: 0x%X ID: 0x%X\n", incomingUBX->cls, incomingUBX->id);
        }
#endif
      }
    ...
    else // Checksum failure
    {
      ...
      if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      {
        ...
        if (debugPin >= 0)
        {
          //gpio_put(debugPin, 0); Commented out to save time for adaptation
          //sleep_ms(10);
          //gpio_put(debugPin, 1);
        }
        ...
        if (_printDebug == true)
        {
            printf("Checksum failed: checksumA: %u checksumB: %u rollingChecksumA: %u rollingChecksumB: %u\n", 
...
sfe_ublox_status_e DevUBLOXGNSS::sendI2cCommand(ubxPacket *outgoingUBX)
{
  ...
  else
  {
    ...
    while (bytesLeftToSend > 1)
    {
      ...
      if (write_bytes(&outgoingUBX->payload[startSpot], x) != x)
        return (SFE_UBLOX_STATUS_I2C_COMM_FAILURE); // Sensor did not ACK
...
void DevUBLOXGNSS::sendSerialCommand(ubxPacket *outgoingUBX)
{
  ...
  // Write payload
  write_bytes(outgoingUBX->payload, outgoingUBX->len);
...
//   write_read_byte(byteToTransfer, &returnedByte);
...
//       {
//         printf("spiTransfer: spiBuffer is full!");
//         printOnce = true;
//       }
// #endif
//     }
...
//     if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
//     {
//       printf("sendSpiCommand: no memory allocation for SPI Buffer!");
//     }
// #endif
  ...
  if (storageNMEAGNZDA != nullptr)                                            // If RAM has been allocated for message storage
    if (storageNMEAGNZDA->callbackCopy != nullptr)                            // If RAM has been allocated for the copy of the data
      if (storageNMEAGNZDA->automaticFlags.flags.bits.callbackCopyValid == 1) // If the copy of the data is valid
      {
        ...
        {
          // if (_printDebug == true)
          //   printf(F("checkCallbacks: calling callbackPtr for GNZDA"));
...
bool DevUBLOXGNSS::pushRawData(uint8_t *dataBytes, size_t numDataBytes, bool callProcessBuffer)
{
  ...
  if (_commType == COMM_TYPE_SERIAL)
  {
    ...
    while (bytesLeftToWrite > 0)
    {
      ...
      ok &= (write_bytes(dataBytes, bytesToWrite) == bytesToWrite); // Will set ok to false if any one write fails
    ...
    else
    {
      ...
      while (bytesLeftToWrite > 0)
      {
        ...
        if (_pushSingleByte == true)
        {
          ...
          bytesWritten += write_bytes(buf, bytesToWrite); // Write the bytes
          dataBytes += bytesToWrite - 1;                 // Point to fresh data
          _pushSingleByte = false;                       // Clear the flag
        }
        else
        {
          bytesWritten += write_bytes(dataBytes, bytesToWrite); // Write the bytes
...
size_t DevUBLOXGNSS::pushAssistNowDataInternal(size_t offset, bool skipTime, const uint8_t *dataBytes, size_t numDataBytes, sfe_ublox_mga_assist_ack_e mgaAck, uint16_t maxWait)
{
  ...
  if (mgaAck == SFE_UBLOX_MGA_ASSIST_ACK_ENQUIRE)
  {
    ...
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
    {
        printf("pushAssistNowData: mgaAck is ENQUIRE. getAckAiding returned %d\n", ackAiding);
    }
#endif
  }
  ...
  while (dataPtr < (offset + numDataBytes)) // Keep going until we have processed all the bytes
  {
    ...
    if (dataIsOK)
    {
      ...
      if ((skipTime) && ((*(dataBytes + dataPtr + 3) == UBX_MGA_INI_TIME_UTC) || (*(dataBytes + dataPtr + 3) == UBX_MGA_INI_TIME_GNSS)))
      {
        ...
        if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
        {
            unsigned char byteValue = *(dataBytes + dataPtr + 3);
            printf("pushAssistNowData: skipped INI_TIME ID 0x%02X\n", byteValue);
        }
#endif
      }
      else
            ...
            if (packetUBXMGAACK->head != packetUBXMGAACK->tail) // Does the MGA ACK ringbuffer contain any ACK's?
            {
              ...
              if (dataAckd) // Is this the ACK we are looking for?
              {
                ...
                else
                {
                  ...
                  if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
                  {
                      printf("pushAssistNowData: packet was _not_ accepted. infoCode is %d\n", packetUBXMGAACK->data[packetUBXMGAACK->tail].infoCode);
                  }
#endif
                }
                keepGoing = false;
    ...
    else
    {
      ...
      if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      {
        printf("pushAssistNowData: bad data - ignored! dataPtr is ");
        printf("%d\n", dataPtr);
      }
#endif
  ...
  if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
  {
    printf("pushAssistNowData: packetsProcessed: ");
    printf("%d\n", packetsProcessed);
  }
#endif
...
//         if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
//         {
//           printf("findMGAANOForDate: found date match at location "ss);
//           printf("%d\n", dataPtr);
//         }
// #endif
//         dateFound = true;
...
//       if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
//       {
//         printf("findMGAANOForDate: bad data - ignored! dataPtr is ");
//         printf("%d\n", dataPtr);
//       }
// #endif
    ...
    while (packetUBXMGAACK->head != packetUBXMGAACK->tail) // Does the MGA ACK ringbuffer contain any data?
    {
      ...
      else if (idMatch)
      {
        ...
        if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
        {
            ...
            printf("readNavigationDatabase: unexpected ACK received. databaseEntriesRX is 0x%X. msgPayloadStart is 0x", databaseEntriesRX);
            
            // Iterate through msgPayloadStart in reverse order to match the original behavior
            for (uint8_t i = 4; i > 0; i--)
            {
                uint8_t value = packetUBXMGAACK->data[packetUBXMGAACK->tail].msgPayloadStart[i - 1];
                // Check if padding is needed
                if (value < 0x10) printf("0");
                // Print the value in hexadecimal
                printf("%X", value);
            }

            // Complete the message with a newline character for consistency and readability
            printf("\n");
        }
#endif
      }
...
bool DevUBLOXGNSS::createFileBuffer(void)
{
  ...
  if (_printDebug == true)
  {
    printf("createFileBuffer: fileBufferSize is: ");
    printf("%i\n", fileBufferSize);
  }
#endif
...
bool DevUBLOXGNSS::createRTCMBuffer(void)
{
  ...
  if (rtcmBuffer == nullptr) // Check if the new (alloc) was successful
  {
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
    {
      printf("createRTCMBuffer: RAM alloc failed!");
    }
    ...
    case TF002_TF006:
      ...
      if (frameCount == 2)
      {
        ...
        if (uSpartnCrc4(&spartn[1], 3) == _header.frameCRC)
        {
          ...
          if (_printDebug == true)
          {
              printf("SPARTN Header CRC is valid: payloadLength %d EAF %d crcType %d\n", 
        ...
        else
        {
          ...
          if (_printDebug == true)
          {
            printf("SPARTN Header CRC is INVALID");
          }
#endif
        }
      }
    ...
    case TF009:
      ...
      if (frameCount == TF007toTF016)
      {
        ...
        else
        {
          ...
          if (_printDebug == true)
          {
              printf("SPARTN authenticationIndicator %d\n", _header.authenticationIndicator);
          }
#endif
          ...
          if (_printDebug == true)
          {
              printf("SPARTN embeddedApplicationLengthBytes %d\n", _header.embeddedApplicationLengthBytes);
          }
#endif
        }
        parseState = TF016;
          ...
          switch (_header.crcType)
          {
            ...
            case 2:
            {
              ...
              else
              {
                ...
                if (_printDebug == true)
                {
                    printf("SPARTN CRC-24 is INVALID: 0x%X vs 0x%X\n", expected, crc);
                }
#endif
              }
            }
...
bool DevUBLOXGNSS::setValFloat(uint32_t key, float value, uint8_t layer, uint16_t maxWait)
{
  if (sizeof(float) != 4)
  {
    ...
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
    {
      printf("setValFloat not supported!");
    }
#endif
    return false;
  }
...
bool DevUBLOXGNSS::setValDouble(uint32_t key, double value, uint8_t layer, uint16_t maxWait)
{
  if (sizeof(double) != 8)
  {
    ...
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
    {
      printf("setValDouble not supported!");
    }
#endif
    return false;
  }
...
#ifndef SFE_UBLOX_REDUCED_PROG_MEM
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      printf("addCfgValsetN: packetCfgPayloadSize reached!");
#endif
    return false;
  }
...
#ifndef SFE_UBLOX_REDUCED_PROG_MEM
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      printf("addCfgValsetN: key limit reached!");
#endif
    return false;
  }
...
bool DevUBLOXGNSS::addCfgValsetFloat(uint32_t key, float value)
{
  if (sizeof(float) != 4)
  {
    ...
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
    {
      printf("addCfgValsetFloat not supported!");
    }
#endif
    return false;
  }
...
bool DevUBLOXGNSS::addCfgValsetDouble(uint32_t key, double value)
{
  if (sizeof(double) != 8)
  {
    ...
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
    {
      printf("addCfgValsetDouble not supported!");
    }
#endif
    return false;
  }
...
bool DevUBLOXGNSS::addCfgValget(ubxPacket *pkt, uint32_t key) // Add a new key to an existing UBX-CFG-VALGET ubxPacket
{
  ...
  if (_lenCfgValGetResponse >= (_cfgValgetMaxPayload - (4 + (valueSizeBytes))))
  {
    ...
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
    {
      printf("addCfgValget: packetCfgPayloadSize reached!");
    }
#endif
    return false;
  }
  ...
  if (_numGetCfgKeys == CFG_VALSET_MAX_KEYS)
  {
    ...
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
    {
      printf("addCfgValget: key limit reached!");
    }
#endif
    return false;
  }
...
#ifndef SFE_UBLOX_REDUCED_PROG_MEM
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      printf("setAutoRXMSFRBXmessageCallbackPtr: RAM alloc failed!");
#endif
    return (false);
  }
...
#ifndef SFE_UBLOX_REDUCED_PROG_MEM
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      printf("setAutoRXMMEASXcallbackPtr: RAM alloc failed!");
#endif
    return (false);
  }
...
#ifndef SFE_UBLOX_REDUCED_PROG_MEM
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      printf("initPacketUBXRXMMEASX: RAM alloc failed!");
#endif
    return (false);
  }
...
#ifndef SFE_UBLOX_REDUCED_PROG_MEM
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      printf("setAutoTIMTPcallbackPtr: RAM alloc failed!");
#endif
    return (false);
  }
...
#ifndef SFE_UBLOX_REDUCED_PROG_MEM
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      printf("initPacketUBXTIMTP: RAM alloc failed!");
#endif
    return (false);
  }
...
#ifndef SFE_UBLOX_REDUCED_PROG_MEM
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      printf("setAutoMONHWcallbackPtr: RAM alloc failed!");
#endif
    return (false);
  }
...
#ifndef SFE_UBLOX_REDUCED_PROG_MEM
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      printf("initPacketUBXMONHW: RAM alloc failed!");
#endif
    return (false);
  }
...
#ifndef SFE_UBLOX_REDUCED_PROG_MEM
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      printf("initStorageNMEA: RAM alloc failed!");
#endif
    return (false);
  }
...
#ifndef SFE_UBLOX_REDUCED_PROG_MEM
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      printf("initStorageRTCM: RAM alloc failed!");
#endif
    return (false);
  }
...
#ifndef SFE_UBLOX_REDUCED_PROG_MEM
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      printf("setRTCM1005callbackPtr: RAM alloc failed!");
#endif
    return (false);
  }
...
#ifndef SFE_UBLOX_REDUCED_PROG_MEM
    if ((_printDebug == true) || (_printLimitedDebug == true)) // This is important. Print this if doing limited debugging
      printf("initStorageRTCM1005: RAM alloc failed!");
#endif
    return (false);
  }
